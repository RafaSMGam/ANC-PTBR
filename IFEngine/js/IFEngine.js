class IFEngine{
	...
	async entra(labelStanza){
		...
		if(this.stanzaCorrente.label !== undefined){
			await this.CRT.println("<strong style='text-decoration:underline'>"+this.stanzaCorrente.label.toUpperCase()+"</strong>");
		}
	...
	async restore() {
		...
		} while (etichetta == i18n.IFEngine.questions.cancelLetter.toLowerCase() || etichetta == i18n.IFEngine.questions.listLetter.toLowerCase())
		
		let realLabel = this.SAVED+"-"+etichetta;
		if(this.storage[realLabel] === undefined){
			await this.CRT.printTyping(i18n.IFEngine.warnings.notFound(etichetta.toUpperCase())+"\n");
			return false;	
		}
