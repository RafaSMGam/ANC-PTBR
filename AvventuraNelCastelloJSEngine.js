class AvventuraNelCastelloJSEngine extends IFEngine{
	...
	async offesa(offesa){
		...
		for(let i=0; i<3000; i++){
			...
			this.CRT.fixed.append(span);
			if(i%10 == 0) {
				await this.s4();
				await this.CRT.sleep(25);
			}
		}
		document.querySelectorAll(".joke").forEach( el => el.remove() );
	...
	async fakeInput(){
		...
		await this.CRT.print(this.defaultInput);
		this.CRT.cursor.classList.add("blinking");
		window.scrollTo(0,document.body.scrollHeight);
		await this.CRT.keyPressed();
		this.CRT.cursor.classList.remove("blinking");
		return;
	}
